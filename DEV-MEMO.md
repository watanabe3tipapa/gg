...
14. **WebGL 非対応環境でも viewer が動くよう修正（2026-09-15, commit `e4ee212`）** — Safari プライベートブラウズで「ページがすぐに初期化されてしまう」症状を調査。原因は `new THREE.WebGLRenderer()` が WebGL コンテキスト生成失敗時に例外を投げ、`GGApp` コンストラクタ全体が中断 → タブ/フォーム初期化が実行されず status が「準備中...」のまま固まること。修正: ①`GraphRenderer` が `try/catch` で WebGL 生成失敗を `this.supported=false` に吸収し、共通フィールド（raycaster / nodePositions 等）は描画依存でないため常に初期化、`controls` 未生成でも `render()/resize()/animate()` は安全に no-op 化。②`app.js` が `supported=false` 時に status-bar に「3D 描画を表示できません ※ WebGL が利用できないため…（クロール・統計は引き続き動作します）」の warning を表示。Playwright 実ブラウザ検証: WebGL 有効（swiftshader）でデモ 19 ページ/20 リンク描画成功、WebGL 無効でも pageerror なし・統計表示成功。GH Pages / CF Pages 両方に反映済み。`viewer/` → `astro/public/viewer/` に同期済み。
