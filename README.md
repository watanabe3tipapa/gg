...
- クローラーは **静的な HTML の `<a href>` のみ**を取得します。JavaScript で動的に生成されるリンク（SPA・クライアントサイドレンダリング）は**検出できません**。
  - 例: `watanabe3ti.com` は JS 描画サイトのため、リンク 0 件になります（正常動作）。
  - この場合、ビューアに警告が表示されます。
- 同一ページの所得結果は 1 ノードのみになる場合があります。
